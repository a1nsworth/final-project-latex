...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Функции принадлежности лингвистических термов истинности. Авторский материал}
\end{figure}
...
\end{itemize}  
\begin{enumerate}  
  \item[] На практике обучение крупных нейросетей нередко разбивают на две части:  
...
\paragraph{Обнаружение и устранение выбросов}
...
\end{equation}
\textbf{Премущества:} простота реализации, гарантированное покрытие решётки.  \\
\textbf{Недостатки:} экспоненциальный рост числа испытаний при увеличении \(k\) и \(n_i\).
...
\end{equation}
\cite{bergstra2012random} показали, что при фиксированном \(N\) Random Search часто эффективнее Grid Search, так как распределяет усилия по более важным измерениям.
...
\paragraph{Современные фреймворки}
...
\begin{center}
\captionof{listing}{Пример использования Optuna}
\begin{minted}{python}
import optuna
